...
class InteractiveShell(SingletonConfigurable):
    ...
    def __init__(self, ipython_dir=None, profile_dir=None,
                 ...
                 custom_exceptions=((), None), log=None, **kwargs):

        # This is where traits with a config_key argument are updated
        # from the values on config.
        self.log = log
        self.log.info("start InteractiveShell ctor")
        super(InteractiveShell, self).__init__(**kwargs)
        
        self.configurables = [self]

        # These are relatively independent and stateless
        self.init_ipython_dir(ipython_dir); self.log.info('self.init_ipython_dir(ipython_dir)')
        self.init_profile_dir(profile_dir); self.log.info('self.init_profile_dir(profile_dir)')
        self.init_instance_attrs(); self.log.info('self.init_instance_attrs()')
        self.init_environment(); self.log.info('self.init_environment()')
        
        # Check if we're in a virtualenv, and set up sys.path.
        self.init_virtualenv(); self.log.info('self.init_virtualenv()')
        
        # Create namespaces (user_ns, user_global_ns, etc.)
        self.init_create_namespaces(user_module, user_ns); self.log.info('self.init_create_namespaces(user_module, user_ns)')
        ...
        self.save_sys_module_state(); self.log.info('self.save_sys_module_state()')
        self.init_sys_modules(); self.log.info('self.init_sys_modules()')
        
        # While we're trying to have each part of the code directly access what
        # it needs without keeping redundant references to objects, we have too
        # much legacy code that expects ip.db to exist.
        self.db = PickleShareDB(os.path.join(self.profile_dir.location, 'db')); self.log.info("self.db = PickleShareDB(os.path.join(self.profile_dir.location, 'db'))")
        
        self.init_history(); self.log.info('self.init_history()')
        self.init_encoding(); self.log.info('self.init_encoding()')
        self.init_prefilter(); self.log.info('self.init_prefilter()')
        
        self.init_syntax_highlighting(); self.log.info('self.init_syntax_highlighting()')
        self.init_hooks(); self.log.info('self.init_hooks()')
        self.init_events(); self.log.info('self.init_events()')
        self.init_pushd_popd_magic(); self.log.info('self.init_pushd_popd_magic()')
        # self.init_traceback_handlers use to be here, but we moved it below
        # because it and init_io have to come after init_readline.
        self.init_user_ns(); self.log.info('self.init_user_ns()')
        self.init_logger(); self.log.info('self.init_logger()')
        self.init_builtins(); self.log.info('self.init_builtins()')
        
        # The following was in post_config_initialization
        self.init_inspector(); self.log.info('self.init_inspector()')
        # init_readline() must come before init_io(), because init_io uses
        # readline related things.
        self.init_readline(); self.log.info('self.init_readline()')
        ...
        self.init_completer(); self.log.info('self.init_completer()')
        # TODO: init_io() needs to happen before init_traceback handlers
        # because the traceback handlers hardcode the stdout/stderr streams.
        # This logic in in debugger.Pdb and should eventually be changed.
        self.init_io(); self.log.info('self.init_io()')
        self.init_traceback_handlers(custom_exceptions); self.log.info('self.init_traceback_handlers(custom_exceptions)')
        self.init_prompts(); self.log.info('self.init_prompts()')
        self.init_display_formatter(); self.log.info('self.init_display_formatter()')
        self.init_display_pub(); self.log.info('self.init_display_pub()')
        self.init_data_pub(); self.log.info('self.init_data_pub()')
        self.init_displayhook(); self.log.info('self.init_displayhook()')
        self.init_latextool(); self.log.info('self.init_latextool()')
        self.init_magics(); self.log.info('self.init_magics()')
        self.init_alias(); self.log.info('self.init_alias()')
        self.init_logstart(); self.log.info('self.init_logstart()')
        self.init_pdb(); self.log.info('self.init_pdb()')
        self.init_extension_manager(); self.log.info('self.init_extension_manager()')
        self.init_payload(); self.log.info('self.init_payload()')
        self.hooks.late_startup_hook(); self.log.info('self.hooks.late_startup_hook()')
        self.events.trigger('shell_initialized', self); self.log.info("self.events.trigger('shell_initialized', self)")
        atexit.register(self.atexit_operations); self.log.info('atexit.register(self.atexit_operations)')
        self.log.info("end InteractiveShell ctor")
